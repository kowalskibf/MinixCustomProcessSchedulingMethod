...
FORWARD _PROTOTYPE(int do_get_process_priority, (message * m_ptr));
FORWARD _PROTOTYPE(int do_set_process_priority, (message * m_ptr));
FORWARD _PROTOTYPE(int do_get_process_counter, (message * m_ptr));
...
PUBLIC void sys_task()
{
  ...
  while (TRUE)
  {
    ...
    case SYS_SET_PROCESS_PRIORITY:
      r = do_set_process_priority(&m);
      break;
    case SYS_GET_PROCESS_PRIORITY:
      r = do_get_process_priority(&m);
      break;
    case SYS_GET_PROCESS_COUNTER:
      r = do_get_process_counter(&m);
      break;
    default:
      r = E_BAD_FCN;
    }
...
PRIVATE int do_set_process_priority(m_ptr)
message *m_ptr;
{
  struct proc *process;
  for (process = BEG_PROC_ADDR; process < END_PROC_ADDR; process++)
  {
    if (!(istaskp(process) || isservp(process)))
    {
      if (process->p_pid == m_ptr->m1_i1)
      {
        process->priority = m_ptr->m1_i2;
        return OK;
      }
    }
  }
...
PRIVATE int do_get_process_priority(m_ptr)
message *m_ptr;
{
  struct proc *process;
  for (process = BEG_PROC_ADDR; process < END_PROC_ADDR; process++)
...
PRIVATE int do_get_process_counter(m_ptr)
message *m_ptr;
{
  struct proc *process;
  for (process = BEG_PROC_ADDR; process < END_PROC_ADDR; process++)
...
register message *m_ptr; /* pointer to request message */
{
  ...
  rp = proc_addr(m_ptr->PROC1);
  if (isemptyp(rp))
    return (E_BAD_PROC); /* process already dead? */
  assert(isuserp(rp));

  /* MM has finished one KSIG. */
  if (rp->p_pendcount != 0 && --rp->p_pendcount == 0 && (rp->p_flags &= ~SIG_PENDING) == 0)
    lock_ready(rp);
  return (OK);
}
...
register message *m_ptr;
{
  ...
  case T_GETINS: /* return value from instruction space */
    ...
    {
      if ((src = umap(rp, T, TR_ADDR, TR_VLSIZE)) == 0)
        return (EIO);
  ...
  case T_GETDATA: /* return value from data space */
    if ((src = umap(rp, D, TR_ADDR, TR_VLSIZE)) == 0)
      return (EIO);
    phys_copy(src, vir2phys(&TR_DATA), (phys_bytes)sizeof(long));
    break;
  ...
  case T_SETINS: /* set value in instruction space */
    if (rp->p_map[T].mem_len != 0)
    {
      if ((dst = umap(rp, T, TR_ADDR, TR_VLSIZE)) == 0)
        return (EIO);
  ...
  case T_SETDATA: /* set value in data space */
    if ((dst = umap(rp, D, TR_ADDR, TR_VLSIZE)) == 0)
      return (EIO);
    phys_copy(vir2phys(&TR_DATA), dst, (phys_bytes)sizeof(long));
    TR_DATA = 0;
    break;
...
message *m_ptr; /* pointer to request message */
{
  ...
  case SYSGETENV:
  {
    ...
    if (sysgetenv.keylen != 0)
    {
      ...
      if (sysgetenv.keylen > sizeof(key))
        return (EINVAL);
 ...
 *				cause_sig				     *
 *===========================================================================*/
PUBLIC void cause_sig(proc_nr, sig_nr) int proc_nr; /* process to be signalled */
 ...
 *				alloc_segments				    *
 *==========================================================================*/
PUBLIC void alloc_segments(rp) register struct proc *rp;
{
