...
PRIVATE int remapping[7] = {0, 0, 0, 0, 1, 1, 2}; /* zad 2*/
PRIVATE int priority_index = 0;					  /* zad 2*/
 ...
 *				ready					     *
 *===========================================================================*/
PRIVATE void ready(rp) register struct proc *rp; /* this process is now runnable */
{
...
PRIVATE void sched()
{
	...
	register struct proc *current_process, *previous_process;
	...
	while (1)
	{
		current_priority = remapping[priority_index];
		priority_index = priority_index == 6 ? 0 : priority_index + 1;
		current_process = rdy_head[USER_Q];
		while (current_process != NIL_PROC)
		{
			if (current_process->priority == current_priority)
			{
				if (current_process == rdy_head[USER_Q])
				{
				...
				previous_process = rdy_head[USER_Q];
				while (previous_process != NIL_PROC && previous_process->p_nextready != current_process)
					previous_process = previous_process->p_nextready;

				previous_process->p_nextready = current_process->p_nextready;
				current_process->p_nextready = rdy_head[USER_Q];
				rdy_head[USER_Q] = current_process;

				if (current_process == rdy_tail[USER_Q])
					rdy_tail[USER_Q] = previous_process;
